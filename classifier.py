...
def processandtrainmodel():
  ...
  #print ("gurudataasetses",dataset['PartsClassID'])
  ...
  dataset['Channels'].fillna(dataset['Channels'].mean(),inplace=True)
  ...
  #dataset['Customer'].fillna(dataset['Customer'].mean(),inplace=True)
  ...
  return cList, dataset


#sys.exit()

def predict(cList,specs, dataset):
  #recommendations = cList.predict(specs)
  #partclassIdfilter = dataset['PartsClassID'].isin(recommendations)
  #filtereddataset = dataset[partclassIdfilter]
  recoset = list()
  #print("spce remom",cList.predict(specs))
  #recommendations.append(cList.predict(specs)).tolist()
  for c in sorted(cList.keys()):
    print ("Classifier = "+c)
    classifier = cList[c]
    recommendations = classifier.predict(specs)
    recoset.extend(recommendations.tolist())
  partclassIdfilter = dataset['PartsClassID'].isin(recoset)
  print (partclassIdfilter)
  filtereddataset = dataset[partclassIdfilter]
  return filtereddataset['Part Number']
